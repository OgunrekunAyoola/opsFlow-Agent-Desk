...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    async function loadActions() {
      ...
      const res = await fetchWithAccess<{ actions?: UserAction[] }>(
        "/actions?limit=20",
      );
      if (cancelled) return;
  ...
  async function saveProfile(e: React.FormEvent) {
    ...
    const res = await fetchWithAccess<{ error?: string }>(
      "/auth/profile",
      {
        method: "PATCH",
      ...
      <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-white/70">
            Update your name, password, and session.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <section className="rounded-2xl border border-white/10 bg-slate-950/70 p-6 space-y-4 lg:col-span-2">
          ...
          <form onSubmit={saveProfile} className="space-y-4">
            ...
            {profileError && (
              <div className="text-xs text-red-300">{profileError}</div>
            )}
            {profileMessage && (
        ...
        <section className="rounded-2xl border border-white/10 bg-slate-950/70 p-6 space-y-4">
          ...
          <form onSubmit={changePassword} className="space-y-3">
            ...
            {passwordError && (
              <div className="text-xs text-red-300">{passwordError}</div>
            )}
            {passwordMessage && (
              <div className="text-xs text-emerald-300">
                {passwordMessage}
