...
export function TicketCard({ ticket }: { ticket: TicketProps }) {
  return (
    <Link to={`/tickets/${ticket.id}`} className="block group relative bg-white/80 backdrop-blur-sm border border-slate-200/60 rounded-xl p-4 transition-all hover:shadow-lg hover:-translate-y-[2px] hover:border-accent-primary/30 cursor-pointer">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className={cn("w-10 h-10 rounded-full flex items-center justify-center shrink-0", 
          ticket.status === 'new' ? 'bg-blue-50 text-blue-500' :
          ticket.status === 'triaged' ? 'bg-purple-50 text-purple-500' :
          ticket.status === 'replied' ? 'bg-green-50 text-green-500' : 'bg-slate-50 text-slate-400'
        )}>
          {ticket.status === 'new' && <AlertCircle size={20} />}
          {ticket.status === 'triaged' && <Bot size={20} />}
          {ticket.status === 'replied' && <CheckCircle size={20} />}
        ...
        <div className="flex-1 min-w-0 grid grid-cols-12 gap-4 items-center">
          ...
          <div className="col-span-4 flex items-center gap-2">
            <span className={cn("px-2.5 py-1 rounded-full text-xs font-medium border", statusColors[ticket.status])}>
              {ticket.status.toUpperCase()}
            </span>
            <span className={cn("px-2.5 py-1 rounded-full text-xs font-medium border bg-white border-slate-200", priorityColors[ticket.priority])}>
